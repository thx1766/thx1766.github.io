...
void sa(searchable_index * searchableIndex, char * termlist)
{
	...
	listoflist * myllist = malloc(sizeof( listoflist));
	myllist->next = NULL;
	myllist->list = NULL;
	
	listoflist * head = myllist;

	word = strtok(termlist, " ");
	//word should be sa
	...
	while(word != NULL){
		...
		if(word == NULL)
			...
			sortedListPtr * newsl = malloc(sizeof(sortedListPtr));
			so(newsl, searchableIndex, word);
			if(myllist->list == NULL)			
				myllist->list = newsl;
			else{
				listoflist * newlolnode = malloc(sizeof( listoflist));
				newlolnode->next = NULL;
				newlolnode->list = newsl;
				myllist -> next = newlolnode;
				myllist = newlolnode;
			}
		}
	}		

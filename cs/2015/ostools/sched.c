...
/*
 * This is a struct that store the queues(256 queues to be specific)
 */
struct mtlv{
	struct list_head queue[256];
	spinlock_t *lock;
	int active;
...
/*
 * The runqueue that i "learned" from the book with neccessary elements
 */

static struct runqueue{
	int cpu;	
	spinlock_t lock;
	struct task_struct *curr_task, *idle;
	struct mtlv mlfq;
	unsigned long nr_running; //Number of runnable processes in the runqueue lists
}runqueues[NR_CPUS] __cacheline_aligned;

#define cpu_runqueue(cpu) (runqueues+cpu)
...
#efine idle_task(cpu) (init_tasks[cpu_number_map(cpu)])
...
/*
 * The implementation of mtlv and runqueue
 */

static inline void add_task(struct task_struct *p, struct mtlv* mlfq)
{
	list_add_tail(&p->run_list, mlfq->queue+p->prio);
	p->mlfq = mlfq;
	mlfq->is_empty_q[p->prio] = 1;
	mlfq->active++;
}

static inline void del_task(struct task_struct *p, struct mtlv *mlfq)
{
	int temp = p->prio;
	list_del_init(&p->run_list);
	if (list_empty(mlfq->queue + temp))
	{
		mlfq->is_empty_q[temp] = 0;
	}
	mlfq->active--;
}


static inline void add_to_runq(struct task_struct *p, struct runqueue *runq)
{
	runq->nr_running++;
	add_task(p, &runq->mlfq);
}

static inline void rm_from_runq(struct task_struct *p, struct runqueue *runq)
{
	runq->nr_running--;
	del_task(p, &runq->mlfq);
}
...
static inline void reschedule_task(struct task_struct *p)
{
	p->need_resched = 1;
}
...
static inline int try_to_wake_up(struct task_struct * p, int synchronous)
{
	...
	runq = runqueues + 0;
	spin_lock_irqsave(&runq->lock, flags);

	//DDD spin_lock_irqsave(&runqueue_lock, flags);
	...
	if(p->mlfq != NULL)
	{
		if(synchronous)
		{
			...
			add_to_runq(p, runq);
			if((runq->curr_task == runq->idle) || (p->prio < runq->curr_task->prio))
			{
				reschedule_task(runq->curr_task);
			}
		}
		success = 1;
	}

	spin_unlock_irqrestore(&runq->lock, flags);
...
/*
 * This is a function that will be used in timer.
 * It check and change the timer:
 * If the counter == 0, then change the priority of this task and reschedule it
 */

void ticking_check(struct task_struct *p)
{
	struct runqueue *runq = runqueues+0;
	unsigned long flags;
	...
	if(p->prio < 256)
	{
		if(--p->counter <= 0)
		{
			...
			if(p->policy != SCHED_FIFO)
			{
				...
				p->counter = PRIO_TO_TIMESLICE(p->prio);
				add_task(p, &runq->mlfq);
			}
		}
	}
...
inline int wake_up_forked(struct task_struct *p)
{
	struct runqueue *runq = runqueues+0;
	unsigned long flags;
	int success = 0;
	...
	p->state = TASK_RUNNING;
	p->prio--;	//give the forked a bit higher priority than its parent
	if(p->prio < 0)
		p->prio = 0;
	add_to_runq(p, runq);
	success = 1;
...
static inline void set_prio_(struct task_struct *p)
{ 
	struct runqueue *runq = runqueues+0;
	unsigned long flags;
	struct mtlv *mlfq = p->mlfq;
	...
	p->prio = NICE_TO_PRIO(p->nice);
	if(mlfq)			
	{
		add_task(p,mlfq);
		runq->curr_task->need_resched = 1;
	}
	spin_unlock_irqrestore(&runq->lock, flags);
}
			...
			struct sched_param *param)
{
	...
	//spin_lock_irqsave(&runq->lock, runq);
...
void __init sched_init(void)
{
	...
	printk("Initiating the runqueue!");
	struct runqueue* runq = cpu_runqueue(0);
	int nr, i;
	struct mtlv* m = &(runq->mlfq);
	...
	enter_lazy_tlb(&init_mm, current, 0);
}
